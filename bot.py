...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── ПРЕДМЕТИ ─────────────────────────────────────────
        ...
        if s:
            ...
            subject_tutors = [k for k, t in TUTORS.items() if key in t["subjects"]]
            tutor_btns = []
            if subject_tutors:
    ...
    # ── РЕПЕТИТОРИ (список) ──────────────────────────────
        ...
        buttons = [
            ...
            for k in SUBJECTS if any(k in t["subjects"] for t in TUTORS.values())
        ...
        ids = [k for k, t in TUTORS.items() if key in t["subjects"]]
        if ids:
        ...
        ids = [k for k, t in TUTORS.items() if t["branch"] == bid]
    ...
    # ── ТЕСТИ ────────────────────────────────────────────
        ...
        buttons = [
            [InlineKeyboardButton(
                f"{SUBJECTS.get(k, {}).get('emoji','📝')} {t['name']}",
...
async def show_tutor_list(q, ids: list, back_target: str, title: str = "Репетитори"):
    buttons = [
        [InlineKeyboardButton(f"👤 {TUTORS[tid]['name']}", callback_data=f"tutor_{tid}")]
        for tid in ids if tid in TUTORS
